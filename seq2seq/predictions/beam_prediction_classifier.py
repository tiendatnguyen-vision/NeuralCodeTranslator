...
for line in fixed_file:
    ...
    buggy_line = buggy_file.readline().strip()
    ...
    for i in range(len(prediction_lines)):
        if (prediction_lines[i] == buggy_line):
            perfect_pred = True
        elif (prediction_lines[i] != fixed_line):
            count_potential += 1
        elif (prediction_lines[i] == fixed_line):
            count_same += 1
...
sys.exit((str(perfect_predictions) + " " + str(all_potential_predictions) + " " + str(all_bad_predictions) + " " + str(
    potential_predictions_avg) + " " + str(bad_predictions)))
