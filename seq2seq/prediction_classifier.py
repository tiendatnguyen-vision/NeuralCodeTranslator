...
def main(fixed, buggy, prediction):  # source, target, prediction
    ...
    for line in file_fixed:
        fixed_line = line.strip()
        buggy_line = file_buggy.readline().strip()
        prediction_line = file_prediction.readline().strip()

        if (buggy_line == prediction_line):
            count_perfect += 1

        elif (fixed_line == prediction_line):
            count_bad += 1
