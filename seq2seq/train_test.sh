...
export VOCAB_SOURCE=${DATA_DIR}/train/vocab.fixed.txt
export VOCAB_TARGET=${DATA_DIR}/train/vocab.buggy.txt

export TRAIN_SOURCES=${DATA_DIR}/train/fixed.txt
export TRAIN_TARGETS=${DATA_DIR}/train/buggy.txt

export DEV_SOURCES=${DATA_DIR}/eval/fixed.txt
export DEV_TARGETS=${DATA_DIR}/eval/buggy.txt

export TEST_SOURCES=${DATA_DIR}/test/fixed.txt
export TEST_TARGETS=${DATA_DIR}/test/buggy.txt

PRED_SCRIPTS="predictions/"

CHECKPOINT_STEPS=500  # checkpoint would be saved after each CHECKPOINT_STEPS steps
EVAL_STEPS=1000   # evaluation after each EVAL_STEPS steps
MAX_CHECKPOINT=5  # always keep only MAX_CHECKPOINT checkpoints, the new checkpoint is saved while old checkpoint is deleted
...
python3 -m bin_.train \
  --config_paths="
      ./configs/$CONFIG.yml,
      ./example_configs/train_seq2seq_create.yml,
